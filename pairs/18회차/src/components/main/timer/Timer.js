...
export default class Timer {
  constructor(dom) {
    ...
    this.props = {};
    this.timer = {};
    const { driver, navigator } = store();
    this.driver = driver;
    this.navigator = navigator[0];
  ...
  template(props) {
    ...
    return `
        <content>
            ...
            <section class="progressbar-wrap" style="display: ${
              this.driver && this.navigator ? '' : 'none'
            }">
                <div class="roles">
                    <div class="driver" style="left:${progress}%;">
                        <span class="driver-name">${this.driver}</span>
                        <span class="driver-icon">🚗</span>
                    </div>
                    <div class="navigator">
                        <span class="navigator-name">${this.navigator}</span>
  ...
  eventHandler() {
    this.dom.addEventListener('click', ({ target }) => {
      ...
      if (target.textContent === BUTTON_SWAP) {
        const { driver, navigator } = store({
          type: 'swapRole',
          member: { driver: this.driver, navigator: this.navigator },
        });
        this.driver = driver;
        this.navigator = navigator[0];
        console.log(this.driver, this.navigator);
        this.render({
          ...this.initTimer(),
          editable: false,
          timerSet: BUTTON_SETUP,
